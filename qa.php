$(document).ready( function() {
	...
	var quiz_num_questions = 8;
	var slide_width = 355;
	$.getJSON("https://spreadsheets.google.com/feeds/list/0AlvITj7wasAPdFIteUl6TmlRLU90WlRxTGt6Q2dnYnc/1/public/values?alt=json-in-script&callback=?", function(data) {
    quiz_num_questions = data.feed.entry[0].gsx$numquestions.$t;
		...
		$.each(data.feed.entry, function (i, item) {
			...
			quiz_values[question_string].newspol = item.gsx$interestnewsandpolitics.$t;
	...
	$('#quiz-end').click(function() {
		...
		*/
		$('#quiz-questions input:checked').each(function(i, item) {
			...
			newspol += quiz_values[item.id].newspol* 1;
    ...
    var causeScores = new Array(animal, disaster, discrimination, education);
    var causeCodes = new Array(animalCode,disasterCode,discriminationCode,educationCode); 
/*	
